...
> **Analysis:** Comparing evaluators, we see a general agreement on the scores given (across all dimensions) for each test example (with over 85% of the two evaluators' scores being within $\pm$ 1). However, the more standard Cohen's Kappa Inter Annotator Agreement measure (which also accounts for accidental random agreement) shows a generally low agreement metric (0.37 when weighted linearly). Observing the heatmap, this is probably due to ChatGPT generally avoiding extreme scores (1 or 5) while Gemini's scores are more diffuse. These fringe differences (156 counts scored 4 by ChatGPT but 5 by Gemini, 74 counts scored 2 by ChatGPT but 1 by Gemini) contribute partially to the reduction in the Cohen's Kappa. On "collapsing" scores 1 and 5 (shifting all scores to 2, 4 respectively) the metric value jumps to 0.51 (moderate agreement)
...
*   `app-frontend/`: Frontend app coded in React + Typescript. Some scripts used in deployment (AWS lambda scripts, Modal deployment script) are placed in `src/deployment`
...
5) App - AI helped significantly with generating the frontend, modal deployment script, lambda functions, cloudflar turnstile integration etc. based on my design.
